...
import type { Size } from '@/types/index.types'
...
export const calculatePrimaryStroke = (
  primary: string | Record<number, string>,
  strokePercent: number,
) => {
  if (!primary) {
...
export const calculateSecondaryStroke = (
  secondary: string | Record<number, string>,
  strokePercent: number,
) => {
  if (!secondary) {
    // Default gray
